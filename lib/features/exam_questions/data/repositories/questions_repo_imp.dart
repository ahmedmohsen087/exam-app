...
import 'package:exam_app/features/exam_questions/data/data_sources/questions_remote_data_source.dart';
...
class QuestionsRepoImpl extends QuestionsRepo {
  ...
  Future<BaseResponse<List<Question>>> getQuestionsOnExam({
    ...
    final cached = await local.getCachedQuestions(examId);

    if (cached.isNotEmpty) {
      return Success<List<Question>>(data: cached);
    }

    final response = await remote.getQuestionsOnExam(
      token: token,
      examId: examId,
    );
    ...
    switch (response) {
      case Success():
        final questions =
            response.data?.map((e) => e.toQuestion()).toList() ?? [];

        await local.cacheQuestions(questions, examId);
